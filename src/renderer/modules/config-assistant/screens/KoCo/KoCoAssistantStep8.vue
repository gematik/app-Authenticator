...
<template>
  <div class="assistant-content m-4">
    <div class="assistant-heading flex flex-row justify-between">
      {{ $t('config_assistant.koco.step8.title') }}
      <AssistantButton
    ...
    <div>
      <p class="mb-2">
        <strong>{{ $t('config_assistant.koco.step8.step_title') }}</strong>
      </p>
      <p>
        {{ $t('config_assistant.koco.step8.intro') }}
      </p>
      <div class="assistant-content mt-4">
        <label for="auth-type-select" class="block mb-2 font-medium text-gray-700">
          {{ $t('config_assistant.koco.step8.auth_type_label') }}
        </label>
        <select
          id="auth-type-select"
        ...
        >
          <option :value="TLS_AUTH_TYPE.BasicAuth">{{ $t('config_assistant.koco.step8.basic_auth') }}</option>
          <option :value="TLS_AUTH_TYPE.ServerClientCertAuth">
            {{ $t('config_assistant.koco.step8.cert_option_others') }}
          </option>
          <option :value="TLS_AUTH_TYPE.ServerClientCertAuth_Pfx">
            {{ $t('config_assistant.koco.step8.cert_option_p12') }}
          </option>
          <option :value="TLS_AUTH_TYPE.ServerCertAuth">
            {{ $t('config_assistant.koco.step8.no_authentication') }}
          </option>
        </select>
      </div>
    </div>
    <div class="flex flex-row mt-8">
      <div class="w-2/3">
        <p class="mb-2">
          <strong>{{ $t('config_assistant.koco.step8.authentication_description_title') }}</strong>
        </p>
        <ol class="list-decimal list-inside">
          <li class="mb-2">
            {{ $t('config_assistant.koco.step8.authentication_description_1') }}
          </li>
          <li class="mb-2">
            {{ $t('config_assistant.koco.step8.authentication_description_2') }}
          </li>
          <li class="mb-2">
            {{ $t('config_assistant.koco.step8.authentication_description_3') }}
          </li>
        </ol>
      </div>
      <div class="flex flex-col justify-start items-end gap-4 w-1/3">
        <ScreenshotHelper
          image-src="koco_step8_authentication.png"
          :image-description="$t('config_assistant.koco.step8.authentication_screen_hint')"
        />
      </div>
    </div>
  </div>
