...
### Attention 
The host and port need to be configured in the nginx configuration exactly as in the Authenticator host configuration.
The reason is, that the returned connector.sds natively contains real Konnektor URLs to the services.
However, by using the Konnektor Gateway, we want to avoid direct communication with the Konnektor.
That's why it is replaced with the URL of the Konnektor Gateway.
...
`docker run --name api-gateway-nginx  -p 9443:443 -v .\nginx_basic_auth.conf:/etc/nginx/nginx.conf:ro -v .\certificates\konnektor-ca-cert.pem:/etc/nginx/ssl/ca-cert.pem:ro -v .\certificates\server-cert.pem:/etc/nginx/ssl/server-cert.pem:ro -v .\certificates\server-key.pem:/etc/nginx/ssl/server-key.pem:ro --rm -d nginx`
