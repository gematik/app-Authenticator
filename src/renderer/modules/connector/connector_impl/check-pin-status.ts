...
import { XML_TAG_NAMES, XML_ERROR_TAG_NAMES } from '@/renderer/modules/connector/constants';
...
import { TPinStatusResponse, TPinStatusTypes, TSoapFault } from '@/renderer/modules/connector/type-definitions';
...
export const getPinStatus = async (
  ...
  try {
    ...
    let soapFault: TSoapFault | undefined;
    if (!pinStatus) {
      const faultCode = cardHandleParser(pinStatusResp, XML_ERROR_TAG_NAMES.TAG_ERROR_CODE);
      const faultstring = cardHandleParser(pinStatusResp, XML_ERROR_TAG_NAMES.TAG_ERROR_FAULT_STRING);
      if (faultCode || faultstring) {
        soapFault = { code: faultCode, faultstring };
        logger.warn(`SOAP Fault detected - Code: ${faultCode}, faultstring: ${faultstring}`);
      }
    }

    if (returnValue) {
      return { statusResult, pinStatus, soapFault };
    }
